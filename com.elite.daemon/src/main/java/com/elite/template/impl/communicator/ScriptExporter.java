...
public class ScriptExporter {
	 ...
	 * Use this if on the same platform as EliControl
	 */
	 ...
	 * Default is 50000. Change is this port is occupied.
	 ...
	 */
	public int exportIntegerFromEliScript(ScriptCommand command, String variable_name) {
	...
	public double exportDoubleFromEliScript(ScriptCommand command, String variable_name) {
		ScriptCommand newCommand = buildScriptCommandToExport(command, variable_name);
		String reply = readValueFromRobot(newCommand);

		double doubleValue = Double.parseDouble(reply);
	 ...
	 */
	public String exportStringFromEliScript(ScriptCommand command, String variable_name) {
		ScriptCommand newCommand = buildScriptCommandToExport(command, variable_name);
		String reply = readValueFromRobot(newCommand);
	...
	private ScriptCommand buildScriptCommandToExport(ScriptCommand command, String variable_name) {
		...
		/** CR*/
		command.appendLine("socket_send_byte(13,"+RETURN_SOCKETNAME+")");
		/** LF*/
		command.appendLine("socket_send_byte(10,"+RETURN_SOCKETNAME+")");
